...
class GeneralAgent(AsyncMultiStepAgent):
    ...
    async def _step_stream(self, memory_step: ActionStep) -> AsyncGenerator[ChatMessageStreamDelta | ToolOutput]:
        ...
        try:
            if self.stream_outputs and hasattr(self.model, "generate_stream"):
                ...
                # Log LLM output (only in debug mode)
    ...
    async def process_tool_calls(self, chat_message: ChatMessage, memory_step: ActionStep) -> AsyncGenerator[StreamEvent]:
        ...
        async def process_single_tool_call(call_info):
            ...
            # Show tool execution start with enhanced UI
            self.monitor.start_tool_execution(tool_name)
            ...
            try:
                ...
                # Show tool execution completion with enhanced UI
                self.monitor.end_tool_execution(tool_name, duration)
                
                # Show observation with better formatting (only in debug mode)
                from src.utils.debug_config import is_debug_mode
                if is_debug_mode():
                    self.logger.log(
                        Panel(
                            f"📋 [bold blue]Tool Result:[/bold blue] [bold yellow]{tool_name}[/bold yellow]\n"
                            f"⏱️  Duration: [dim]{duration:.2f}s[/dim]\n"
                            f"📄 Output: [dim]{observation[:200]}{'...' if len(observation) > 200 else ''}[/dim]",
                            border_style="blue",
                            padding=(0, 1)
                        ),
                        level=LogLevel.INFO,
                ...
                # Show error with enhanced UI (only in debug mode)
                from src.utils.debug_config import is_debug_mode
                if is_debug_mode():
                    self.logger.log(
                        Panel(
                            f"❌ [bold red]Tool Error:[/bold red] [bold yellow]{tool_name}[/bold yellow]\n"
                            f"⏱️  Duration: [dim]{duration:.2f}s[/dim]\n"
                            f"💥 Error: [dim]{str(e)}[/dim]",
                            border_style="red",
                            padding=(0, 1)
                        ),
        ...
        if final_answer_call:
            ...
            # Show final answer processing start (only in debug mode)
            from src.utils.debug_config import is_debug_mode
            if is_debug_mode():
                self.logger.log(
                    Panel(
                        f"🎯 [bold magenta]Processing Final Answer[/bold magenta]\n"
                        f"📋 Preparing comprehensive response...",
            ...
            if isinstance(answer, str) and answer in self.state.keys():
                ...
                # Log final answer extraction (only in debug mode)
                from src.utils.debug_config import is_debug_mode
                if is_debug_mode():
                    self.logger.log(
                        Panel(
                            f"📤 [bold green]Final Answer Ready[/bold green]\n"
                            f"🔑 Extracted from state key: [bold yellow]{answer}[/bold yellow]\n"
                            f"📄 Content: [dim]{str(final_answer)[:200]}{'...' if len(str(final_answer)) > 200 else ''}[/dim]",
                            border_style="green",
                            padding=(0, 1)
                        ),
                        level=LogLevel.INFO,
                ...
                duration = time.time() - start_time
                # Log final answer generation (only in debug mode)
                from src.utils.debug_config import is_debug_mode
                if is_debug_mode():
                    self.logger.log(
                        Panel(
                            f"🎯 [bold green]Final Answer Generated[/bold green]\n"
                            f"⏱️  Processing time: [dim]{duration:.2f}s[/dim]\n"
                            f"📄 Content: [dim]{str(final_answer)[:200]}{'...' if len(str(final_answer)) > 200 else ''}[/dim]",
                            border_style="green",
                            padding=(0, 1)
                        ),
